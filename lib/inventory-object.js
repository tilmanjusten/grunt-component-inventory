...
module.exports = function (data) {
    ...
    // set default properties (property whitelist)
    var inventoryObject = {
        category:       data.hasOwnProperty('category') ? data.category : 'Not categorized',
        id:             data.hasOwnProperty('id') ? data.id : '',
        lines:          data.hasOwnProperty('lines') ? data.lines : -1,
        name:           data.hasOwnProperty('name') ? data.name : '',
        options:        data.hasOwnProperty('options') ? data.options : {},
        optionsData:    data.hasOwnProperty('optionsData') ? data.optionsData : '',
        origin:         data.hasOwnProperty('origin') ? data.origin : '',
        partial:        data.hasOwnProperty('partial') ? data.partial : '',
        template:       data.hasOwnProperty('template') ? data.template : '',
    ...
    inventoryObject.parseData = function(src, opts) {
        ...
        var idBase = src.replace(/\s+/gi, '');
        // make hash from contentss
        var id = crypto.createHash('sha1').update(idBase, 'utf8').digest('hex');
        ...
        name = blockOpts.hasOwnProperty('name') ? blockOpts.name : _.startCase(blockOpts.extract);

        // set category name
        category = blockOpts.hasOwnProperty('category') ? _.startCase(blockOpts.category) : this.category;
     ...
     * @returns {boolean}
     */
    inventoryObject.setProperty = function (prop, value) {
